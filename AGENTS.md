...
* **安装器验收套件 = `tests/`（常驻 Pester）**：`pwsh -NoProfile -Command "$c = New-PesterConfiguration; $c.Run.Path = 'tests'; $c.Run.Exit = $true; $c.TestResult.Enabled = $false; Invoke-Pester -Configuration $c"`。用例只写 `$env:TEMP`（`tests/TestHelpers.ps1` 的 `New-TestCase` 建临时 home + 三个临时目标 + `claude` shim），并在生成的子 wrapper 内嵌互锁、拒绝指向真实 HOME ——**这是唯一被允许的"验证安装器行为"路径**；`Invoke-Pester -CI` 会按默认 `TestResult.OutputPath` 在 CWD 落 `testResults.xml`；该文件未列入忽略规则，会脏化工作树。
