...
* **本仓的验收套件 = `tests/`（常驻 Pester）**：`pwsh -NoProfile -Command "$c = New-PesterConfiguration; $c.Run.Path = 'tests'; $c.Run.Exit = $true; $c.TestResult.Enabled = $false; Invoke-Pester -Configuration $c"`。用例只写 `$env:TEMP`（`tests/TestHelpers.ps1` 的 `New-TestCase` 建临时 home + 三个临时目标 + `claude` shim），并在生成的子 wrapper 内嵌互锁、拒绝指向真实 HOME ——**这是唯一被允许的"验证安装器行为"路径**；`Invoke-Pester -CI` 会按默认 `TestResult.OutputPath` 在 CWD 落 `testResults.xml`，而本仓无 `.gitignore`，会脏化工作树。
* **派生对漂移门 = `tools/dsh-drift-check.ps1`**：`claude/` 与 `dsh/` 的 17 对派生文件，其全部差异行冻结在 `tools/dsh-drift-baseline.txt`。`pwsh -NoProfile -File tools/dsh-drift-check.ps1` → 退出码 `0` = 差异集合与登记一致；`1` = 出现未登记的新增 / 改动 / 消失行并逐行点名。有意分歧须登记进 `docs/ai/DSH-LANDING-NOTES.md` §6，并在同一 commit 用 `-Update` 重生成基线（默认运行零写入）。
