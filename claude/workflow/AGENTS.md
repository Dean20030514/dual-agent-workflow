...
* **Blocking Issues** 每条标 `[Product Blocking]`（用户可见正确性 / 用户数据错误 / 安全）或 `[Verification Blocking]`（验证不健全：probe 冒充证据、测试不走真实路径、审后弱化测试或改验收），并由 **Reviewer** 标 `caused_by_last_fix: yes/no`（**仅 Critical** ——该字段只服务 Fix-Loop streak；Routine 无此账本，不填）。**只有 Blocking Issues（Product/Verification）默认阻止合并。**（**阻止合并 ≠ 计入硬停**：streak 只由 `[Product Blocking]` 递增，见「Fix-Loop 计数与跨轮硬停」；两审零 Product 时 Verification 的收口方式见「最后一轮独立审查门 → 证据层出口」。）
...
* **`[Verification Blocking]` 的成立门槛（2026-08-30 新增，斩断无反例的散文阻塞）**：每条 Verification Blocking 须**点名具体证据缺口**——哪一条声称 / 由哪份产物或哪个测试支撑 / 缺的是什么——**并给出一个可证伪的检查**：写进 `Verification Needed` 的可执行命令，或一个「若该性质不成立则该检查会失败」的对照样本。**Reviewer 不实跑，故不要求其给出实际运行结果**；缺口与可证伪检查**两者都写不出**的（"覆盖不够全面""建议补充说明 X"），一律降级 Non-Blocking Suggestion，**不得阻止收敛**。本门槛**只约束证据缺口类的成立形式，不改变上条的分类优先级**——有产品 / 安全影响的一律仍按 `[Product Blocking]` 记。
