...
Always launch Codex from Claude Code via the **Bash tool with the prompt passed as a command-line argument**, stdin explicitly sealed. **The complete invocation template (isolation flags, machine key, explicit model/effort, holding paths) lives ONLY in `~/.claude/workflow/reviewer-prompt.md` → 双审隔离协议 ③ — copy it from there, never reconstruct it from memory or from this file.** Every review run — dual 9A/9B, single downgraded 9A, and the 9P plan review alike — writes BOTH the `-o` verdict and the raw log to a holding directory OUTSIDE the repo work tree (Reviewer zero-write, no in-repo exception; the old in-repo single-review paths are retired). Dual review: 9B first, 9A second, work tree verified free of any verdict/log before the second run — otherwise the two verdicts stop being independent.
