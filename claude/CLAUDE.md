...
- **Critical (human-enabled)**: the full workflow in `~/.claude/workflow/` + the 7 slash commands (Frozen Acceptance, 9P plan review, Human Approval gate, SHA binding, 9A/9B dual review, Reviewer zero-write, Fix-Loop hard stop) — all existing mechanisms unchanged.
- **Suggest, don't self-upgrade**: when a task touches auth/permissions/secrets, money/billing, DB migrations or irreversible data ops, deploy/rollback/CI core, public API or compatibility contracts, or spans many architecture layers — recommend Critical and wait for the human's confirmation. A task-level imperative such as "do it", "go ahead", "做吧", or "直接做" does not by itself enable Critical mode. When a Critical trigger applies, stop before modifying files or installing dependencies, recommend Critical, and proceed only after the human explicitly confirms the mode. Enabling Critical does not itself approve an implementation plan; the normal Critical approval gate still applies.
- **Suggest, don't self-downgrade** (symmetric to the clause above): when the deliverable itself is a process / rules / register / self-referential document (the output is mainly text describing this workflow or its own ledgers, and the criteria layer barely changes) — recommend Routine and wait for the human's confirmation; never downgrade on your own. Evidence (measured, the same in both landings): the review surface does not close — every fix grows a new claim to audit; four real projects' Critical tasks **all** ended `stopped, NOT converged` / hard stop, and in one task 100% of the findings across 5 rounds landed in the change-register / AC-predicate layer with 0% in the criteria layer. This pathology was diagnosed on 2026-08-15 ("evidence-layer self-reference spiral") — before the DSH landing and independently of the model. If the human still wants Critical on that shape, the precondition is a frozen delivery boundary (criteria whitelist + machine-readable register); otherwise the convergence gate is unreachable by construction.
...
**Research-reuse-first（先找轮子，再造轮子）**：**新实现、新依赖、架构选择、新协议**四类改动强制先做一次有记录的复用检索；纯文档修正、已定位的本地缺陷修复、沿用仓内既有模式、纯数据改动可**显式**跳过（跳过须给出规范要求的 `skip_reason`）。成熟方案优先**采用 / 移植 / 包装 / 引用**而非从零写，从零自建须写明否决理由；是否采用按需求适配度、维护状态、许可证、集成成本与安全风险综合判断——**不设固定百分比阈值**。**语义、字段形状与校验入口的唯一规范来源 = `~/.claude/workflow-core/reuse/README.md`**（`install.ps1` 部署的核心协议；本文件与 `rules/common/development-workflow.md` 只留触发与指针，不复述细则）。检索结论：Routine 在对话里留**一句 inline 结论**；Critical 写进 `/explore` 的 Reuse Findings 与 `IMPLEMENTATION_PLAN.md` 的 **Reuse / Prior Art** 小节；**Reviewer 不重跑检索**，只核声明与真实使用。
...
- **MCP servers loaded**（2026-07-25 实测快照，**按来源分层**；加载集随客户端版本与插件启停变化，**永远以本次会话 system-reminder 的工具列表为准**）。挑选分层：**dedicated MCP > Chrome MCP > computer-use**。
  - **插件提供**（随该插件卸载/禁用而消失）：`plugin:context7:context7`(库文档) · `plugin:chrome-devtools-mcp:chrome-devtools`(浏览器测量诊断)。`playwright` / `github` 已于 2026-07-25 卸载 —— GitHub 操作走 `gh` CLI；`serena` 已于 2026-07-28 卸载 —— 代码导航走原生 Grep/Glob/Read + `LSP` 工具（语言后端由 pyright-lsp / typescript-lsp / clangd-lsp 插件供给）。
...
- **Sub-agents**: 可用的 `subagent_type` **以本次会话 system-reminder 的 agent 列表为准**（通用型：general-purpose / Explore / Plan 等）。**本机没有领域专家 agent** —— 它们曾由 `ecc` 插件提供（planner / architect / tdd-guide / code-reviewer / security-reviewer / …），该插件已于 2026-07-25 卸载，其 marketplace 配置残留已于 2026-08-05 从 settings.json 清除（历史来源：affaan-m/everything-claude-code，仅作记录）；`~/.claude/agents/` 目录不存在，勿再引用。**没有专家 agent 时，其职责由主 Agent 自己承担，不得跳过对应工作** —— 降级规则见 `~/.claude/rules/common/agents.md`（唯一权威出处）。Use the `Task` / `Agent` tool with `subagent_type` for parallelizable independent work. **Sub-agents must inherit the main conversation's model** — always omit the `model` parameter; never downgrade to a weaker model (haiku/sonnet/etc.), even for mechanical tasks or under cost pressure. Prefer merging dispatch batches or trimming prompts over downgrading (user rule, 2026-06-10; cost rationale in `~/.claude/workflow/workflow-design-notes.md`). **Fan-out cap (user rule, 2026-09-05; applies in every mode, ultracode included — its "token cost is not a constraint" default does not override this):** a workflow run uses at most **10** agents with at most **6** running concurrently, and a turn launches at most **3** workflows. **Never one agent per item** (per file / per finding / per claim) — batch the list into ≤ 10 groups, one agent each. Adversarial verification is one verifier per batch, not N voters per finding; no loop-until-dry. Exceed these only when the user writes an explicit `+Nk` budget in that prompt, and then scale from `budget.total`. After a run dies on a usage limit, shrink before resuming — errored agents are not cached. Origin: 2026-09-03/04 runs of 97 / 69 / 59 agents (one agent per claim, plus one verifier per finding) at ≈25–30万 tokens each burned the quota; the useful runs needed 4–10. **Ultracode and the phase commands (user rule, 2026-09-06):** ultracode changes how much parallelism a phase uses, not whether the phase command runs. In Critical mode still enter every phase through the Skill tool — `/define → /explore → /plan → /implement → /final-review`, plus `/design-check` and `/debug` when their triggers fire — and run any workflow *inside* that phase. A Workflow fan-out is not a substitute for `/explore`, and reading a section of the portable prompt is not a substitute for `/plan`: the command file is what carries the phase's procedure (canonical templates, the 0.1 scan, Reuse Findings, Pre-Flight). Origin: 2026-09-03..06, 20 of 27 ultracode sessions ran a Critical task without loading a single phase command, and the handoff files written from memory drifted from the templates.
...
**Roles:** Claude Code (Author) = understanding / architecture / planning / implementation / test fixing / final review. Codex CLI (Reviewer) = independent **lightweight** review only — the 9P plan review + the 9A/9B implementation reviews (fresh context + a different model's perspective, not doing the Author's work). Human (me) = approve the plan / inspect diffs / decide trade-offs / final commit. The two agents do **not** share memory.
...
| Reviewer prompts 9P/9A/9B + 输出契约 (copy to Codex) | `~/.claude/workflow/reviewer-prompt.md` |
...
`/define (产品定义 + 18 维适用性扫描) → /explore (只读探索) → /plan (写交接文件 → 9P 计划审 → Approval gate) → /implement (实现 + 测试产物到 docs/ai/last_test_run.txt + 适用质量/设计闸门) → Codex 轻量独立 review (用 reviewer-prompt.md) → /final-review (含代跑 Codex 的 Verification Needed) → human commit`. Small tasks **within Critical mode** use the fast path (see `/implement` 末尾「快速版」) and MUST still keep at least `docs/ai/HANDOFF.md` (with the 0.1 scan + approval evidence). Routine tasks (the default) do not enter this flow and carry no handoff-file requirement.

**Six non-negotiable principles** (full 9 条核心原则 in `index.md`): (1) handoff via files, never verbally; (2) verification artifacted — test output to file, next agent reads the file not the prose; (3) git as gate — commit each phase, rollback via revert; (4) `IMPLEMENTATION_PLAN.md` Human Approval Status editable by human only; (5) every change explainable/verifiable/rollbackable; (6) Reviewer is lightweight — no rebuilding copies / reinstalling deps / rerunning full suites (implementation reviews reason from `docs/ai/last_test_run.txt` + git diff; the 9P plan review reasons from the working-tree planning files; both list commands under *Verification Needed* for the Author to run).

**Scaffolding a project:** create repo-root `AGENTS.md` (copy from `~/.claude/workflow/AGENTS.md` template, fill project specifics) + `CLAUDE.md` (one line: `@AGENTS.md`) + `docs/ai/HANDOFF.md`. Codex 与 Claude Code 两侧均已于 2026-07-28 卸载 Superpowers（Claude 侧纪律由自家 workflow 承载，见「Process discipline」节；Codex 侧靠 9P/9A/9B prompt 自带的完整审查契约）—— 两边靠 `docs/ai/` 交接文件 + workflow 对齐，不靠插件对齐；原「Codex 缺能力时回流 `docs/ai/HANDOFF.md`」的特例依旧不需要。 **便携单文件版**（**只给装不了结构化工作流的外机用**）：`C:\Users\16097\Desktop\通用prompt-v3.8.txt`（`@` 一个文件即得全流程；规范源在快照仓 `portable/通用prompt-v3.8.txt`，桌面副本须与其保持哈希一致）。母本改规则后，便携版**整体重新生成**，勿逐条打补丁。**本机会话不附加它**（2026-09-06 裁决）：结构化母本已装（项目 `AGENTS.md` + 本文件 + 7 个命令），附加便携版只会多出一条「读节代替调命令」的路，每会话还多塞 160 KB；若某会话仍出现 `@便携版`，以结构化母本与 7 个命令为准，便携版仅作对照。

**Discipline points (anti-rationalization):** systematic-debugging (root cause first), claiming-completion gate (evidence before "done"/"passes"), receiving-review discipline (verify a Reviewer's points against the code before acting; no performative agreement), excuse→reality tables, Cannot-Verify-From-Diff verdict, Pre-Flight Plan Review. These live in the `/implement` `/final-review` `/debug` commands + `~/.claude/workflow/reviewer-prompt.md` (9P/9A/9B contracts) + `workflow-design-notes.md` (为何外部制衡之外还要内部自律). （原对应的 Superpowers 技能已随插件卸载（2026-07-28）——命令文件自身即全量定义，无外部技能依赖。）

### Codex invocation (durable, all projects)

> **Which prompt to send:** `reviewer-prompt.md` (9P/9A/9B, dual-review isolation, snapshot self-check, SHA binding) is **Critical-only**. For a **Routine ad-hoc review** — a human asking for one independent read without enabling Critical — do **not** send that template: it would make the Reviewer demand handoff files Routine does not have and must not create. State the mode in the prompt, name the files/diff to review, and use the Routine evidence-carrier sentence from `~/.claude/workflow/AGENTS.md` → Reviewer-Lightweight Protocol layer 2. The invocation flags below are the same in both cases.

Always launch Codex from Claude Code via the **Bash tool with the prompt passed as a command-line argument**, stdin explicitly sealed. **Invocation templates live ONLY in `~/.claude/workflow/reviewer-prompt.md` — copy from there, never reconstruct from memory or from this file: the 9A/9B template (isolation flags, machine key, explicit model/effort, holding paths) is in 双审隔离协议 ③, and the 9P command is in the 9P section — the two run at different reasoning efforts, so reusing ③'s template for a plan review is wrong.** Every review run — dual 9A/9B, single downgraded 9A, and the 9P plan review alike — writes BOTH the `-o` verdict and the raw log to a holding directory OUTSIDE the repo work tree (Reviewer zero-write, no in-repo exception; the old in-repo single-review paths are retired). Dual review: 9B first, 9A second, work tree verified free of any verdict/log before the second run — otherwise the two verdicts stop being independent.

`-o, --output-last-message <FILE>` writes ONLY Codex's final message (the structured review verdict) — that `.md` is the file Claude reads back, NOT the raw log; the shell redirect captures the full process log (reasoning, tool-call summaries, shell output, error stacks) for debugging only. Read the verdict first; open the raw log only when the verdict is empty/missing (hang or hard kill) or you are diagnosing a failure. The `-o` file is written only when Codex finishes — to watch live progress, tail the raw log. (Prepend `export PATH="/c/Users/16097/bin:$PATH"` when pnpm is needed.) Never spawn it from the PowerShell tool. Hard-won specifics (24-min idle hang 2026-06-11, recurred across projects):
...
**Codex review prompts — lean protocol (mandatory, all projects; quota incident 2026-06-10):** the protocol has **two layers**, and **`~/.claude/workflow/AGENTS.md` → Reviewer-Lightweight Protocol is the single definition — this file does not restate the criteria.** Layer 1 is the always-on semantics (no rebuilt copies, no reinstalled deps, no rerunning full suites; list what needs real execution for the Author). Layer 2 is the **evidence carrier and it is mode-scoped** — the Critical implementation review vs the Critical plan review (9P — planning files only, no test artifacts yet) vs a **Routine ad-hoc Reviewer** (a human asking for one review without enabling Critical; this does not upgrade the mode, and such a Reviewer must never be pointed at handoff files Routine does not have and must not create). **Copy the ready-made sentence for the mode at hand from the AGENTS.md section — do not assemble it here**, and note the carrier sits mid-sentence, so layer 1 is not a standalone contiguous string to grep for. The 9P/9A/9B prompts in `reviewer-prompt.md` are Critical-only and already inline the matching carrier sentence — copy and it's included. Rationale: Codex's sandbox cannot read the repo's `node_modules` (EPERM) and will otherwise rebuild a full mirror (`git archive` + `pnpm install`, tens of thousands of files, GBs of disk) and rerun entire suites at `xhigh` reasoning on every round — one day of reviews exhausted the whole Codex quota. If Codex insists verification is needed, it must LIST the commands; Claude runs them in a normal terminal and appends the real output to `docs/ai/last_test_run.txt` — or declines an item with a technical reason; the human sees each item before merge (Critical implementation review), uses it to revise the plan (9P — no `last_test_run.txt` exists yet), or pastes it back into the conversation (Routine). Delete any leftover `review-*` / `.codex-review-*` scratch dirs after human confirmation.
