...
- DSH 短输入仍走命令行；长输入通过运行专属 patch 的原生 `headless-runner.config.task` 传入，完整规则与 diff 不截断。70,426 字符的真实传输探针已通过，见 LONG_INPUT_TRANSPORT。完整输入仍有 UTF-8 字节上限，日志只提供带哈希摘录；未声称 DSH CLI 新增了文件/stdin 参数，也不把传输探针当业务审查通过。
