...
## 3. 收回后必须核的 6 项（第 4 项与第 4b 项都是**自报值**；后果分级见下）

> **缺项 / 不符的后果分级（2026-09-16 人类裁决）**：第 4 项与第 4b 项都是自报值——契约原文就写着"自报值、不是证据"。因此分两种情形，别混：
> ① **Reviewer 如实申报了一项被禁止的行为**（例如"我发生过写入"）→ **该轮作废**（已申报的违反就是已发生的违反）；
> ② **只是漏写字段，或 `model_route` 与实发参数不符** → **记 Work Log + 报告人类**，Author 落账时补记，**不作废**。
> **作废的另一来源是独立核验**：启动前扫描发现工作树/holding 残留（`reviewer-prompt` ④）、`observed_head_sha` 不符、两份 `worktree_clean` 非 `yes`、`read_handoff_from` 出现 `git show tip`（该项申报的同样是禁止行为，归 ①）。
> **理由**：把"漏写一行"也定成作废，既抓不到不诚实的 Reviewer，又会因一行漏写废掉一份合规的独立判断；真正有牙齿的是 ① 与那几项独立核验。
...
| 4 | `writes_performed` | 两份都是 `none`；**如实申报了任何写入尝试 → 该轮作废重跑**（申报的即已发生的违反；DSH 无沙箱兜底）。**漏写该行 → 记 Work Log + 报告人类**（自报值缺失不是违反） |
