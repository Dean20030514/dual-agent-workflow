...
**两个选项与后果**：
* **选项 1（Author 推荐）：收窄声称** —— 把 AC4 的域明确写成"只覆盖 `reasoning_effort: <未加引号小写>` 的**赋值位**；散文式与驼峰式取值不在覆盖内"。**代价**：AC4 从"全域声称"降为"赋值位声称"（但仍保留 `[M]`——有命令、有退出码、有负向对照）。**收益**：与谓词实际能力一致，且**不引入新的自由文本判据**（本任务六轮来反复出错的那一类）。
* **选项 2：加宽正则** —— 覆盖散文式与驼峰式，各补一条负向对照。**代价**：散文匹配天然脆（本轮 Author 自己的枚举脚本就因行号读错而失败一次）；会把"文档措辞"变成门禁对象。

**Author 建议选项 1**，依据全部来自上表的量化产物，不是推理。
