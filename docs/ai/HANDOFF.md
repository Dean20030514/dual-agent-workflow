...
[DEBT] AC4-门的覆盖边界：它只枚举"赋值位"里的一种**拼写**（`reasoning_effort: <未加引号小写>`），而取值**还可以以散文式**（`` 9A/9B = `high` ``）**或驼峰赋值位**（`reasoningEffort: …`）出现——那两类不在域内、门不看（2026-09-06 第 6 轮 9B 的 R6-S1 + 第 7 轮量化，原始枚举见 `last_test_run §AM–§AN`：域内 `reasoning_effort|reasoningEffort` 共 **20 处提及 / 18 行**，其中 **7 行是键名提及或占位模板、本非赋值位**，真正"陈述取值"的是 **4 行**且取值全为 `high`；**`reasoning_effort:` 拼写的赋值位 7/7 全被覆盖、当前无活假绿**）| Payback trigger: 下次改动该脚本或 AC4 声称之前 | Impact: 处置方向待人类裁决（`HANDOFF` → 裁决矩阵 §二 的选项 1 / 2a / 2b）；**未处置前不得合并**——存在"官方键名拼写 `reasoningEffort:` 可写 `medium` 走绿"这一条已知逃逸面
...
| A3 | 第 5 轮 9A-B2（AC4 门只覆盖声明域内的 7 处） | **缺陷仍在**（真逃逸形态存在）；**风险面已量化** | **是**（唯一实质项） | 见下方 §二。分类（Product/Suggestion）决定 streak 与合并门，且处置方向未定。 |
...
| # | 岔口 | 需要人判吗 | 说明与建议 |
|---|---|---|---|
| B1 | 任务 B 的交付边界是否现在冻结 | **是** | 第 6 轮 9B 的 Requirement Concern 3：不冻结则"收口"没有可复现判据。**建议冻结为**：`§2.3` 登记表 + AC4/AC6 两个 `[M]` 门 + AC9 的 `[O]` 读点 + 唯一债台账（`HANDOFF`）。超出此范围的一律归 `[U]`。 |
