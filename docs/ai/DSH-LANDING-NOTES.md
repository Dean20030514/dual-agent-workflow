...
| **D2** | `workflow/reviewer-prompt.md` → ③(c) 证据面一致；`skills/independent-review/SKILL.md` §3 | **自报字段缺项的后果**：DSH 侧由「缺一项 → 该轮作废重跑」降级为「记 Work Log + 报告人类」；**作废只留给经独立核验确认的违反**（残留扫描 / `observed_head_sha` / `worktree_clean` / `read_handoff_from`） | **母本没有这两个字段**（`writes_performed` / `model_route` 是 DSH 落地新增，见 §2 第 7 条），因此也没有对应条款可同步；而且"把作废建在自证上"这个缺陷只存在于 DSH 侧新增的那段规则里 |
...
4. **顺带取回一条当轮真缺陷（此前无人报过）**：9B 轮 3 指出当轮 `install.ps1` 的 `Backup-IfExists`（**`-Recurse`**）被施加于 skill bundle **目录** → 产生 `~/.dsh/skills/<bundle>.bak-<stamp>/`，而 `dsh-skill-filesystem` 只把扫描根**直接**的 `<name>/SKILL.md` 视为 skill（嵌套 `**/SKILL.md` 故意不发现）→ 该备份目录**符合发现模式**，会作为第三个（陈旧的）skill 出现。**我复核：机制在当轮 tip 上成立**（tip 的 `install.ps1:33-36/103`），**当前 HEAD 已顺带消除**（新 `Backup-FileIfExists` 要求 `-PathType Leaf`，备份落在 bundle 内、不匹配发现模式），本机实测 `~/.dsh/skills` 下 0 个 `*.bak-*`。⇒ 登记为**已被重写消除的历史缺陷**，不新开债。**以下为随后对 HEAD 的复核（本节其余 finding 同此处理）**：

5. **对 tip 成立、对 HEAD 不成立的收尾（2026-09-16 逐条复核）**：本节与 §7.1 引用的"**10/10 复现**"四条账本/措辞类 finding，**其对象都是被审 tip（`34b60370`），不是当前 HEAD**。逐条回查 HEAD：`added only when missing` 全仓零命中（install.ps1 重写时删净）；README 的「未经 9P/9A/9B 审查」「三笔」零命中（h3c 收口已改准）；`DSH-LANDING-NOTES.md` 的旧轮次状态零命中（已是"已跑 3 轮…采纳 9B"终版）。⇒ **不要再按这些行去改今天的树**。
6. **10 轮 verdict 里被引最多的那笔债（`Unpaid`）在 HEAD 上已闭合**：债文 = "`install.ps1` 的 DSH 段把整个 `~/.dsh`（含 `sessions/`/`storages/`/`.credentials.yaml`）整树备份"。HEAD 实测**已不存在**——`install.ps1` 头部明写 "no whole-tree backup: only the individual files it overwrites are copied, **because a tree-wide backup copied credentials and had no rollback value**" 与 "no copy of credentials, sessions, storages, profiles or settings files"；全文件**只有一个** `Backup-*` 调用点（`Backup-FileIfExists -Path $Target`，且该函数要求 `-PathType Leaf`，不碰目录）；并有用例守着（`tests/install.Deploy.Tests.ps1:65` "does not touch machine-local files: no write, no backup"，断言 `.credentials.yaml.bak-*` 计数为 0）。⇒ **该债在 2026-09-15 的重写中已偿还**；重放把它判 `Unpaid` 是对 tip 的正确读数、对 HEAD 的过期读数。
7. **§7.1/§7.2 两条结构性建议的处置（2026-09-16 人类裁决）**：① "AC1 应改成『允许差异白名单 + 脚本判红』的可执行形态" —— **已落地**，即 `tools/dsh-drift-check.ps1` + `tools/dsh-drift-baseline.txt`（它偿还得正是 AC1 那条债，见 §6 引言）；② "`model_route` / `writes_performed` 不应以缺项即作废" —— **已改**，见 §6 **D2**。
