...
## Proposed Changes
逐文件：文件路径、改动类型(新增/修改/删除)、计划内容、原因、风险。**清单必须与 TASK_BRIEF「Change Budget & Allowed Paths」一致**——预算 SSOT 在 TASK_BRIEF（9B 可见），本文件只解释实施方式，不重复定义预算（见 workflow/AGENTS.md → 改动面预算）。
