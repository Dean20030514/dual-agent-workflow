...
**收敛预算（规则见 `AGENTS.md` → 改动面预算）**：读 HANDOFF 的 Change Budget Ledger——有效任务审查轮达到阈值仍未收敛 → 只能冻结「新可选要求/新超界加固」、保留全部未解决 blocking 原样上交人类（收敛裁决 / 重新规划 / 换上下文）；**冻结不封缺陷**（违反既有验收 / Safety Rules / 真实用户与数据安全仍必须报告）；禁止降级缺陷或据此标 Ready to Commit（`stopped, NOT converged` 语义不变）。
...
2. 任务分支 diff ⊆ 批准 pathspec（正式 = TASK_BRIEF「Change Budget & Allowed Paths」；快速版：按 AGENTS 临时 fail-safe（触及 review_sensitive_paths 即升级正式流程）；标准流程产物已显式纳入、无隐式除外集——语义见 `AGENTS.md` → 改动面预算），无无关修改。
