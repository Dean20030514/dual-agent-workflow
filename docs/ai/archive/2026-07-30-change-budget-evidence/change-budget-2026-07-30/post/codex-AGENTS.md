...
Temporary fail-safe: the quick-version budget contract is being finalized by a
separate task; until then any quick task touching `review_sensitive_paths` must
be upgraded to the formal flow (TASK_BRIEF budget), and the quick-version
code-review path is suspended.
...
- Respect the approved change budget when reviewing: the TASK_BRIEF's "Change
  Budget & Allowed Paths" and "Threat Model" sections. Specifics arrive inlined
  in each review prompt.
