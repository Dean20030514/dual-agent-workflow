...
4. 完成后（**顺序严格**，让 `tested_sha` 真含本次实现）：
   ...
   3. **确认 review-sensitive 路径干净**：`git status --porcelain -- <HANDOFF 的 review_sensitive_paths>` 为空（非空则回 1，先把改动纳入 commit）。**并做预算自检**：全量 `git status --porcelain` + diff 对照 TASK_BRIEF「Change Budget & Allowed Paths」的允许 pathspec——白名单外改动 = 扩面（批准 pathspec 已含标准流程产物，无隐式除外集）→ 停止并按 `AGENTS.md` → 改动面预算处置（回 /plan 经人类重新批准）。
...
> **改动面预算快速版（临时 fail-safe；唯一定义处：`AGENTS.md` → 改动面预算）**：快速版预算契约由独立任务定稿中；在此之前按唯一定义处的临时 fail-safe 执行（触及 `review_sensitive_paths` 即升级走正式流程——TASK_BRIEF 预算；快速版代码审查路径暂停）。
...
2. Author 输出简短计划（可只在对话中），等人类一句话批准（临时 fail-safe：触及 `review_sensitive_paths` 则升级走正式流程——见 `AGENTS.md` → 改动面预算）。
