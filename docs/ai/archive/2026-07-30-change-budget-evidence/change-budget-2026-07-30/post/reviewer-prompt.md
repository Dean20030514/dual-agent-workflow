...
（快速版代码审查路径暂停——临时 fail-safe 见 AGENTS.md → 改动面预算。）
...
3. **改动面预算核对（机械）**：对照批准预算逐项核对 diff（正式 = TASK_BRIEF「Change Budget & Allowed Paths」）——白名单外文件改动、或任何已批准预算维度变化（新文件/模块/公共接口、新架构层、新依赖/lockfile/迁移；LOC 量级除外——仅预警）→ 判 [Verification Blocking]（批准 pathspec 已含标准流程产物——显式纳入、无隐式除外集；语义见 AGENTS.md → 改动面预算）。并核对是否破坏现有 API/数据结构。
4. 安全、边界遗漏、类型、测试覆盖不足。
...
威胁模型定界（语义见 AGENTS.md → 改动面预算）：安全类发现以威胁模型为解释边界（正式 = TASK_BRIEF「Threat Model」节）——超出定界的可选加固默认记入 Non-Blocking Suggestions 并交人类，不判 Blocking（不占用 Debt Verdict 的 Noted 值）；但以下仍判 Blocking：证明威胁模型本身错误（附证据）/ 违反 AGENTS Safety Rules / 违反 QUALITY_GATES 适用安全闸门 / 违反你可见且有权核对的批准验收（9A = TASK_BRIEF Acceptance + PLAN Frozen Acceptance）/ 真实用户与数据安全问题。
...
改动面预算核对（机械）：对照批准预算核对 diff（正式 = TASK_BRIEF「Change Budget & Allowed Paths」）——白名单外文件改动、或任何已批准预算维度变化（新文件/模块/公共接口、新架构层、新依赖/lockfile/迁移；LOC 量级除外——仅预警）→ [Verification Blocking]（批准 pathspec 已含标准流程产物——无隐式除外集）。威胁模型定界：安全类发现以威胁模型为解释边界（正式 = TASK_BRIEF「Threat Model」节）——超出定界的可选加固默认记入 Non-Blocking Suggestions 并交人类（不占用 Debt Verdict 的 Noted 值）；但以下仍判 Blocking：证明威胁模型本身错误（附证据）/ 违反 AGENTS Safety Rules / 违反 QUALITY_GATES 适用安全闸门 / 违反你可见且有权核对的批准验收（9B = TASK_BRIEF Acceptance；你不读 PLAN）/ 真实用户与数据安全问题。
