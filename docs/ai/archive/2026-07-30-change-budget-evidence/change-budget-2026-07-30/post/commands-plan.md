...
4. 不实现代码。计划采用**满足 Frozen Acceptance 的最小完整方案**——既不选漏需求/会返工的偷懒小方案，也不为抽象/统一而扩大范围；**扩大架构必须有证据且经人类批准**（反的是"偷懒选差方案"，不是"越大越好"）。「最小」约束**实现范围**（不夹带无关改动/重构）。必须有测试计划（命令须真实存在）。**计划自检**：通读确保无自相矛盾步骤、无与 TASK_BRIEF 验收冲突、无"计划要求但会被 Reviewer 判缺陷"处——让实现期 `/implement` 的 Pre-Flight 能一次通过。
   ...
   * **改动面预算与威胁模型（起草义务）**：在 `TASK_BRIEF.md` 起草「Change Budget & Allowed Paths」与「Threat Model」两节（骨架见 `templates/TASK_BRIEF.md`；语义唯一定义处 = `workflow/AGENTS.md` → 改动面预算），随计划一并交人类批准——预算 SSOT 在 TASK_BRIEF（9B 可见），PLAN 的 Proposed Changes 清单须与其一致；小任务减负规则见唯一定义处。
...
* **Awaiting Approval**：提醒人类——检查计划 → **先**在 `IMPLEMENTATION_PLAN.md` 填 `Status: Approved / Approved by / Date` → **再** `git add` 计划文件一并 commit（批准状态因此在批准 commit 内、工作树干净；该 commit 即批准凭证）→ 通知 Author 开始 `/implement`。
