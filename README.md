...
| `core/reuse/` | **每个 home 一份**：`~/.claude/workflow-core/reuse/`、`~/.codex/workflow-core/reuse/`、`~/.dsh/workflow-core/reuse/` | **「先找轮子」语义的唯一规范来源**（`README.md` = 语义与字段形状，`reuse.schema.json` = 结构契约，`Reuse.ps1` = 纯校验器；无网络、无子进程、无新依赖）。Claude / DSH / Codex 三种入口与 Team 只**引用**、不复制规则；同一份内容部署到三个互不重叠的受管文件夹，任一入口读到的是同一语义，`Get-ReuseProtocolIdentity` 的身份哈希可跨副本比对（内容一致即身份相同）。**边界**：它不改变任何权威、权限、验收或 Git 政策，`deviation` 只是声明、不构成批准 |
