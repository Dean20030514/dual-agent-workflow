...
> **DSH 侧迁移状态（2026-09-06；本轮次账在 `docs/ai/HANDOFF.md`）**：`dsh/` 与 `portable/通用prompt-DSH-v1.txt` 由 `claude/workflow/` 母本**派生**（骨骼机械复制 + 有界 DSH 化改点，改点清单与机读登记表见 `docs/ai/DSH-LANDING-NOTES.md` §2 / §2.1 / §2.2 / §2.3）。**已跑 3 轮独立双审**：第 1 轮两份"不通过"（7 条 Product Blocking）、第 2 轮两份"有条件通过"、第 3 轮 9B"不通过"（1 条：AC6 改写后按字面仍红）而 9A 判同一条为 Suggestion → **归类分歧经人类裁决采纳 9B**（2026-09-06），已进入第 4 轮修补。`~/.dsh/{AGENTS.md,workflow,skills/{dual-agent-workflow,independent-review}}` 已由**人工等价部署**落位（`install.ps1` 受迁移期 guard 锁定、未运行；该副本没有脚本会产生的 `*.bak-*`，改仓后也不会自动跟随——须由 Author 手工同步）。**当前状态 = 已生效、已审过但未收敛的候选 overlay**：`[DEBT]` 四笔按 No-Hidden-Debt 登记在 `docs/ai/DSH-LANDING-NOTES.md` §3（第 4 笔已按人类裁决偿还、仅余残余风险）。**在收敛门通过前不得标"已收敛"或 Ready to Commit。**
