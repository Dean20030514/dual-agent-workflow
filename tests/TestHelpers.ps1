...
function New-TestCase {
    ...
    if ($SeedTargets) {
        ...
        Set-Content -Path (Join-Path $case.ClaudeDir 'workflow\stray.md') -Value 'live-only file a deploy must report as stale and never delete' -Encoding ascii
...
function New-ChildWrapper {
    ...
    param(
        ...
        [switch]$WithoutClaudeCli,
        [int]$PluginTimeoutSec = 0
    )
    ...
    if ($PluginTimeoutSec -gt 0) {
        # Shortens the installer's per-plugin-install timeout so the suite can exercise the
        # stall path without waiting the production 180s.
        $lines.Add("`$env:INSTALL_PS1_PLUGIN_TIMEOUT_SEC = '$PluginTimeoutSec'")
    }
    if ($WithoutClaudeCli) {
        # Strip PATH down to the system directories: no shim, and no real claude either.
        # The guard below then proves the real CLI is unreachable, so a case that wants the
        # "no CLI" branch can never accidentally trigger a real plugin install.
        $lines.Add("`$env:PATH = '`$env:SystemRoot\System32;`$env:SystemRoot'")
    } else {
    ...
    $lines.Add('}')
    if ($WithoutClaudeCli) {
        $lines.Add('if (Get-Command claude -ErrorAction SilentlyContinue) { Write-Output ("INTERLOCK-FAIL claude-reachable=" + (Get-Command claude).Source); exit 97 }')
    } else {
    ...
    $lines.Add("Write-Output ('CHILD-ENV userprofile=' + `$env:USERPROFILE + ' claude=' + `$(if (Get-Command claude -ErrorAction SilentlyContinue) { (Get-Command claude).Source } else { '<none>' }) + ' targets=' + (`$targets -join ',') + ' psver=' + `$PSVersionTable.PSVersion.Major + ' host=' + (Get-Process -Id `$PID).Path)")
...
function Invoke-InstallerCase {
    ...
    $wrapper = New-ChildWrapper -Case $Case -HostExe $HostExe -OverrideTargets $OverrideTargets -WithoutClaudeCli:$WithoutClaudeCli -PluginTimeoutSec $PluginTimeoutSec
...
function Get-ManagedDeploySet {
    ...
    foreach ($skillDir in (Get-ChildItem -LiteralPath (Join-Path $RepoRoot 'dsh\skills') -Directory)) {
        foreach ($f in Get-ChildItem -LiteralPath $skillDir.FullName -File -Recurse) {
...
function Get-TaggedLines {
    <# Lines emitted with an exact output-contract tag, e.g. '[STALE]'. #>
