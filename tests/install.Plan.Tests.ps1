...
Describe 'AC2 -DryRun' {
    ...
    It 'lists the live-only path it would delete, and the leaf-name look-alike' {
        $deleteLines = Get-TaggedLines -Output $script:R.Output -Tag '[DELETE]'
        @($deleteLines | Where-Object { $_ -like ('*' + (Join-Path $case.ClaudeDir 'workflow\stray.md') + '*') }).Count | Should -BeGreaterThan 0
        @($deleteLines | Where-Object { $_ -like '*myarchive.md*' }).Count | Should -BeGreaterThan 0 -Because 'a leaf name containing archive is NOT a path segment'
    }
    ...
    It 'preserves every keep-local-only sample, including archive as the last segment' {
        ...
        $delete = Get-TaggedLines -Output $script:R.Output -Tag '[DELETE]'
        @($delete | Where-Object { $_ -match '\\archive(\\|$)' }).Count | Should -Be 0 -Because 'only a path SEGMENT named archive is keep-local; myarchive.md is not'
        # Review 9A round 3, G2/G3: the DIRECTORY form must be asserted too (the \\archive
        # filter above cannot see "[DELETE] …\archive (dir)"), and *.bak-* must be undeletable.
        ...
        @($delete | Where-Object { $_ -like '*archive (dir)' }).Count | Should -Be 0 -Because 'a keep-local-only directory must never be planned for deletion'
        @($delete | Where-Object { $_ -like '*.bak-*' }).Count | Should -Be 0 -Because 'a *.bak-* leaf name is keep-local-only as well'
    }

    It 'covers exactly the managed surface of the old script (A3) and keeps config.toml out of [DELETE]' {
        ...
        @(Get-TaggedLines -Output $script:R.Output -Tag '[DELETE]' | Where-Object { $_ -like '*config.toml*' }).Count | Should -Be 0
    }
    ...
    It 'summary counters equal the rows actually printed (Amendment ㉚)' {
        $m = [regex]::Match($script:R.OutputText, 'planned=(\d+) delete=(\d+) preserve=(\d+)')
        $m.Success | Should -BeTrue -Because 'the counter line is part of the frozen output contract'
        $deleteRows = @(Get-TaggedLines -Output $script:R.Output -Tag '[DELETE]').Count
        ...
        [int]$m.Groups[2].Value | Should -Be $deleteRows -Because 'delete=0 while [DELETE] rows are on screen under-reports the blast radius'
        [int]$m.Groups[3].Value | Should -Be $preserveRows
        [int]$m.Groups[1].Value | Should -BeGreaterThan 0
        [int]$m.Groups[2].Value | Should -BeGreaterThan 0 -Because 'this case seeds stray.md, so there is something to delete'
        [int]$m.Groups[3].Value | Should -BeGreaterThan 0 -Because 'this case seeds archive/ and *.bak-* samples'
    }

    It 'negative control: a single test-side write DOES change the signature (the probe is not vacuous)' {
        Set-Content -Path (Join-Path $case.ClaudeDir 'written-by-the-test.txt') -Value 'x' -Encoding ascii
        (Get-TargetsSignature -Case $case) | Should -Not -Be $script:Before
    }

    It 'a preserved DIRECTORY keeps its subtree: no [DELETE] row may target its contents (9A round 4 B1)' {
        ...
        $delete = Get-TaggedLines -Output $r.Output -Tag '[DELETE]'
        @($delete | Where-Object { $_ -like ('*' + $bakDir + '*') }).Count | Should -Be 0 -Because 'the whitelist is subtree-inheriting: a directory that stays keeps its whole content'
    }

    It 'invariant (9B round 5 TCG-1 / AC2 assertion 6): no [DELETE] (dir) row may be an ancestor of a [PRESERVE] row' {
        ...
        foreach ($delLine in $delete) {
            $delPath = (($delLine -replace '^\[DELETE\]\s+', '') -replace ' \(dir\)$', '')
            foreach ($preLine in $preserve) {
                $prePath = (($preLine -replace '^\[PRESERVE\]\s+', '') -replace ' \(dir\)$', '')
                $prePath.StartsWith($delPath + '\', [System.StringComparison]::OrdinalIgnoreCase) | Should -BeFalse -Because "the plan must be a partition, but a row below the deleted '$delPath' is preserved as '$prePath'"
            }
        }
    }
