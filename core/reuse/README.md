...
| `Reuse.ps1` | 纯校验器（dot-source 使用；不联网、不起子进程、不写文件，但**会设置** `Set-StrictMode -Version Latest` 与 `$ErrorActionPreference = 'Stop'`，这是 dot-source 的会话级副作用） |
...
- `strategy ∈ {adopt, port, wrap, reference, build}`；**仅对 `applicability = required` 的决策**，非 `build` 策略必须存在至少一个 `candidate.decision` 与之一致的候选（被选中者）；`applicability = skipped` 时不做此检查（此时 `candidates` 必须为空）。
...
- 候选按**需求适配度、维护状态、许可证、集成成本与安全风险**综合判断；本协议只冻结字段形状与必填理由，**不设固定百分比阈值**——成熟方案优先**采用 / 移植 / 包装 / 引用**，从零自建（`build`）须写明否决理由。
...
- **dot-source 有会话级副作用**：`Reuse.ps1` 在文件级设置 `Set-StrictMode -Version Latest` 与 `$ErrorActionPreference = 'Stop'`，调用方会一并继承；除此之外它没有写文件、子进程或网络行为，文件读取只有第 7 节列出的两类。
